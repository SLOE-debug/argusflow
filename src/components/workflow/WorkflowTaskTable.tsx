    ...
    <tr className="h-[37px] border-b border-slate-200 bg-white hover:bg-blue-50/30">
      ...
      <td className="px-3">
        <div className="flex items-center gap-3 text-slate-600">
          {task.status === 'failed' ? (
            <RotateCw className="size-3.5" aria-label="重新运行" />
          ) : (
            <Play className="size-3.5" aria-label="运行任务" />
          )}
          {task.status === 'running' ? (
            <Pause className="size-3.5" aria-label="暂停任务" />
          ) : task.status === 'success' ? (
            <BarChart3 className="size-3.5" aria-label="查看结果" />
          ) : null}
          <MoreHorizontal className="size-4" aria-label="更多操作" />
        </div>
      </td>
    </tr>
  );
