...
export function useTimelineScale(
  ...
  const interval = Math.max(
    step,
    ([1, 2, 5, 10].find((multiple) => multiple * magnitude >= rough) ?? 10) *
      magnitude,
  );
  const subdivisions =
    interval / 10 >= step ? 10 : interval / 5 >= step ? 5 : 1;
  const ticks = Array.from(
    { length: Math.floor((maximum / interval) * subdivisions) + 1 },
    (_, index) => (index * interval) / subdivisions,
  );
