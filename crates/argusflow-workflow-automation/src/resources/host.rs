...
pub struct AutomationHost {
    ...
    /// 可选共享剪贴板观察服务，由宿主关闭。
    #[cfg(windows)]
    pub clipboard: Option<argusflow_windows::ClipboardReader>,
}
...
impl QuerySourceProvider for StaticSource {
    fn resolve<'a>(&'a self, _: &'a Operation) -> TaskFuture<'a, QuerySource> {
...
impl QuerySourceResource {
    ...
    /// 每次使用前解析最新范围。
    pub fn resolve<'a>(&'a self, operation: &'a Operation) -> TaskFuture<'a, QuerySource> {
        self.0.resolve(operation)
    }
}
