...
use argusflow_core::{
    AppSession, AutomationError, AutomationExecutionScope, BrowserSession,
    PreparedAutomationTarget, PreparedTargetLocator, PreparedVisualPostcondition, ResourceTypeId,
    TargetLocator, TargetScope, UiPostcondition, VisualQuery,
};
...
pub(super) fn resolve_postcondition(
    ...
    Ok(Some(match postcondition {
        UiPostcondition::NewText {
            ...
            query: resolve_visual_query(query, context)?,
            stable_context: stable_context
                .iter()
                .map(|query| resolve_visual_query(query, context))
                .collect::<Result<Vec<_>, _>>()?,
        },
        UiPostcondition::TextPresent { query } => PreparedVisualPostcondition::TextPresent {
            query: resolve_visual_query(query, context)?,
        },
    }))
}

/// 冻结一条视觉查询中的动态文字值，后端不再访问流程表达式环境。
fn resolve_visual_query(
    query: &argusflow_core::VisualQueryExpr,
    context: &RunContext,
) -> Result<VisualQuery, RuntimeError> {
    Ok(VisualQuery {
