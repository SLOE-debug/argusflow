...
- 摄入线程立即读取输入窗口与键盘布局，并按隐私配置复制当前版本剪贴板。输入目标与事件时间保持不变，主要截图由独立有界线程执行操作后采样，与 UIA/CDP 并行。
- 独立桌面观察线程最多约 30Hz，画面更新先经过分块精确 diff，再向全部待处理事件发布同一个变化代数。输入线程不再截图。键盘/窗口至少 500ms、点击至少 250ms，并要求观察到变化后的安静区间达到 150ms；超时保存结果并标记未确认稳定。点击前目标从有界历史按真实输入时间选取，与操作结果分文件保存；换窗口后的结果不能套用原点击标记。算法、基准与限制见 [screen-diff.md](screen-diff.md)。
...
- PNG 压缩/写入在独立有界线程中执行，最多排队四帧。左键按下 crop 从同一操作后帧裁切，最大 192×192，边缘按实际图像范围缩小。停止时排空已接收的采样与编码任务，不在打开历史时补拍。
...
Hook 输入队列 4096，异步事件队列 2048，并发观察上限 16，整理前最多保留 100,000 个原始事件。操作后采样待处理上限 16，提交队列上限 16，每个任务最多 20Hz；队列满或超出调度预算时报告截图缺失。晚于事件 150ms 才摄入的事件不启动补拍或结构化检查。结构化查询期间输入窗口或焦点发生变化时撤销该元素；操作后图像保留自己的实际采样时间和耗时，不冒充输入时刻像素。
...
键盘仍使用目标线程布局与无副作用 `ToUnicodeEx`。默认不自动遮盖原始输入、剪贴板、结构化名称或截图。用户可在录制前显式开启敏感数据遮盖，独立选择敏感/未知字段键盘输入、剪贴板内容、全部截图；会话开始后锁定配置。键盘遮盖开启时，释放继承按下状态，安全组合键仍保留。截图保护采用不采集、不保存整帧及 crop 的方式，不声称自动识别图像敏感区域。共享 UIA/CDP 观察层只报告敏感性，由录制会话决定是否清空名称。
...
控制命令为 `start_recording`、`stop_recording`、`get_recording_status`、`list_recordings`、`get_recording`；`read_recording_screenshot` 只接受录制 UUID、事件序号和截图类型，通过固定文件名与路径归属检查读取 PNG，不能读取任意路径。

### 录制后的隐私检查

录制器以全屏面板展示，右上角“×”只收起面板，不停止录制。界面使用“操作记录”称呼事件。录制前默认保留原始内容；保存后通过“检查隐私内容”打开独立 Dialog。

- 隐私面板使用底部时间轴回看，不执行全量 OCR。拖动浏览，切换“选择片段”后可多次拖动选择不连续区间，也可选择全部；重叠区间自动合并。
- 画面按已保存截图的实际采样时间切换，下一采样之前持续显示上一帧，首帧之前明确显示无采样。后台持续采样历史目前仅驻留内存，并非完整视频归档。
- 按键与组合键在画面底部居中显示，新按键淡入、旧按键上移淡出；鼠标按屏幕物理坐标显示位置、点击高亮与滚动提示。回看不执行真实键鼠输入。
- `edit_recording_privacy` 接收强类型批量操作：`erase_event` 删除整条记录及四类截图；`mosaic` 接收完整帧原图坐标矩形，使用与原像素无关的马赛克替换区域，并从处理后的图像重建局部放大图。
- 片段支持删除操作记录、仅删除按键与剪贴板记录（及所属截图）、遮盖全部截图，也可单张手动框选。截图处理包含延续进入所选片段的上一帧；界面说明不同处理的保留内容。保存前确认具体数量与不可恢复性；已分享的副本不会改变。
- 处理成功后刷新时间线、历史计数及截图 URL，复制和导出使用更新后的内容。后端先校验整批事件和矩形，编辑与历史读取互斥。多文件写入不构成文件系统事务；磁盘故障可能留下部分已处理内容，界面提示重新打开录制检查，不报告整批成功。

界面默认展示操作后完整截图、采样时间与稳定状态。左键按下在完整图上叠加点击位置框，按点击邻域实际像素亮度对比选择非红色标记，并在远离点击的一侧叠加局部放大图。原始 PNG 不烧入标记；缩放与负屏幕原点通过统一物理像素坐标处理。分页只影响展示，复制/导出包含完整 JSON 和证据引用。**JSON 本身不是完整多模态输入**：交给 AI 时需要一并提供 PNG，保留相对路径和事件关联。收起面板继续录制，使用“停止并保存”结束监听。录制与工作流运行互斥沿用已有控制面。
