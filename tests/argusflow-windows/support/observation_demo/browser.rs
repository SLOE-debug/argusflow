...
impl Browser {
    pub async fn start(runtime: &Path, run: &Path, marker: &str) -> Result<Self> {
        ...
        options.arguments = vec![
            runtime.join("observation-driver.mjs").display().to_string(),
            pipe_name,
            super::native::find_chrome()?.display().to_string(),
            profile.display().to_string(),
            marker.into(),
            run.join("browser.jsonl").display().to_string(),
        ];
        let application =
    ...
    async fn receive(&mut self) -> Result<Value> {
        ...
        match response {
            value if value["type"] == "error" => Err(value["message"]
                .as_str()
    ...
    pub async fn shutdown(&mut self) -> Result<()> {
        let close = self.request(serde_json::json!({"type":"close"})).await;
        self.application
